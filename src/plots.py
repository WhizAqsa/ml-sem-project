...
def plot_histogram(dataset):
    dataset.hist(bins=50, figsize=(30,26))
    plt.tight_layout()
    plt.show()

# scatter plot of the dataset
def visualize_scatter_plot_without_smote(X, y):
...
# scatter plot of the dataset
def visualize_scatter_plot_with_smote(X, y):
    pca = PCA(n_components=2)
    X_pca = pca.fit_transform(X)
    plt.scatter(X_pca[:, 0], X_pca[:, 1], c=y, cmap="viridis")
    plt.xlabel("Principal Component 1")
...
def visualize_confusion_matrix(cm):
    ...
    sns.heatmap(
        ...
        fmt=".2f",
        cmap="Blues",
        xticklabels=["Cotton", "Rice"],
        yticklabels=["Cotton", "Rice"],
    )
...
def plot_sample_area_curve(sample_no, dataset):
    sample_data = dataset.iloc[sample_no][
        [
            "NDVI01",
            "NDVI02",
            "NDVI03",
    ...
    plt.fill_between(
        time_steps, sample_data_array, alpha=0.3
...
def plot_hierarchical_class_distribution_in_clusters(labels, clusters, n_clusters):
    ...
    for cluster in range(n_clusters):
        ...
        label_counts = {
            label: np.sum(labels[cluster_indices] == label) for label in unique_labels
        }
...
def plot_hierarchical_clusters_without_pca(X, clusters, n_clusters):
    ...
    for cluster in range(n_clusters):
        cluster_indices = np.where(clusters == cluster)[0]
        plt.scatter(
            X[cluster_indices, 0], X[cluster_indices, 1], label=f"Cluster {cluster}"
        )
