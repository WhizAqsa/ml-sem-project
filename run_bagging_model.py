...
def run_bagging_model():
    ...
    for split_name, X_train, y_train, X_test, y_test in splits:
        ...
        print(f"Bagging Best params: {best_params_bagging}")
        best_bagging_model = BaggingClassifier(random_state=42, **best_params_bagging)

        print("Running the best bagging classifier...")
        y_pred_bagging_best = fit_and_get_predictions(
            best_bagging_model, X_train, y_train, X_test
        )

        # print the classification report for the best model
        score, eval_report, cm = get_eval_metrics(y_test, y_pred_bagging_best)
